...
export default function Home() {
    ...
    try {
      return (
        <div className="md:ml-[80px]" style={{ minHeight: '100vh', background: '#F0F9FF' }}>
          ...
          {/* Kindness Connect FAB - Only show for high school */}
          {!isMiddleSchool && (
            <>
              <div
                onClick={() => setIsKindnessConnectOpen(true)}
  ...
  return (
    <div className="md:pl-[80px]" style={{ 
      ...
      {/* Share Kindness Floating Action Button - POSITIONED TO THE RIGHT OF FEED */}
      {isAuthenticated && (
        <div
          onClick={() => setIsPostModalOpen(true)}
          ...
          style={{
            left: 'clamp(calc(50% + 300px + 2rem), calc(50% + 300px + 2rem), calc(100vw - 6rem))'
          }}
        >
          <button
