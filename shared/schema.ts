...
// 🏫 ECHODEED FOR SCHOOLS PLATFORM TABLES
export const schools = pgTable("schools", {
  ...
  schoolType: varchar("school_type", { length: 50 }), // elementary, middle, high, k-12
  gradeRange: varchar("grade_range", { length: 20 }), // K-5, 6-8, 9-12, etc.
  studentCount: integer("student_count"),
  teacherCount: integer("teacher_count"),
  principalId: varchar("principal_id").references(() => users.id),
  settings: jsonb("settings"), // School-specific configuration
...
export const schoolUsers = pgTable("school_users", {
  ...
  role: varchar("role", { length: 20 }).notNull(), // student, teacher, admin, principal
  gradeLevel: varchar("grade_level", { length: 10 }), // K, 1, 2, etc.
  className: varchar("class_name", { length: 100 }), // Mrs. Smith's 3rd Grade
  studentId: varchar("student_id", { length: 50 }), // School's internal student ID
  parentEmail: varchar("parent_email", { length: 200 }), // For parent notifications
  isActive: integer("is_active").default(1).notNull(),
  joinedAt: timestamp("joined_at").defaultNow().notNull(),
});

export const schoolKindnessPosts = pgTable("school_kindness_posts", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  postId: varchar("post_id").notNull().references(() => kindnessPosts.id),
  schoolId: varchar("school_id").notNull().references(() => schools.id),
  studentUserId: varchar("student_user_id").notNull().references(() => users.id),
  teacherApproved: integer("teacher_approved").default(0), // 0=pending, 1=approved, -1=rejected
  approvedBy: varchar("approved_by").references(() => users.id),
  approvedAt: timestamp("approved_at"),
  points: integer("points").default(10), // Kindness points earned
  isClassroomActivity: integer("is_classroom_activity").default(0),
  assignmentId: varchar("assignment_id").references(() => kindnessAssignments.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const kindnessAssignments = pgTable("kindness_assignments", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  schoolId: varchar("school_id").notNull().references(() => schools.id),
  teacherId: varchar("teacher_id").notNull().references(() => users.id),
  title: varchar("title", { length: 200 }).notNull(),
  description: text("description"),
  category: varchar("category", { length: 50 }), // helping, sharing, caring, etc.
...
export const studentKindnessPoints = pgTable("student_kindness_points", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  studentUserId: varchar("student_user_id").notNull().references(() => users.id),
  schoolId: varchar("school_id").notNull().references(() => schools.id),
  ...
  level: integer("level").default(1).notNull(), // Kindness level 1-10
  badges: jsonb("badges"), // Array of earned badge IDs
  lastActivity: timestamp("last_activity").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const schoolClassrooms = pgTable("school_classrooms", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  schoolId: varchar("school_id").notNull().references(() => schools.id),
  teacherId: varchar("teacher_id").notNull().references(() => users.id),
  ...
  totalKindnessPoints: integer("total_kindness_points").default(0),
  isActive: integer("is_active").default(1),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
